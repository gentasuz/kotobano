    ...
    try {
        const response = await axios.post(
            URL,
            {
                "model": "gpt-3.5-turbo",
                "messages": [
                    { "role": "system", "content": "あなたはカウンセラーです。userから現在の体調に関する文章が送られてくるので、反応してください。"},
...
const chatList = {
    1: {text: '「ことばのまとい」へようこそ', continue: true, option: 'normal'},
    ...
    5: {text: '今日の体調はいかがですか？', continue: false, option: 'normal'},
    // userCount3：体調
    6: {text: '', continue: true, continue: false, option: 'normal'},
...
function textSpecial() {
    chatList[4].text = `こんにちは！${userData[0]}さん！`;
...
if (userCount === 2 || userCount === 3 || userCount === 4) {
    await updateChatListWithGpt3Response(userText.value, 6);
    await updateChatListWithGpt3Response(userText.value, 7);
    await updateChatListWithGpt3Response(userText.value, 10);
}
